...
bool solve(int target_sum, int sum, int arr[], int n, vector<int>& ds, int index) {
    ...
    sum += arr[index];
    if (solve(target_sum, sum, arr, n, ds, index + 1)) {
        return true;
    }
...
int main() {
    ...
    // Call the function with the size of the array (5 in this case)
